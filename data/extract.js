...
function extract(db, dataset) {
	...
	collection.find().each(function(err, doc) {
		if (!doc) {
			fs.writeFile("json/" + args._[0] + ".json", JSON.stringify(counties));
			db.close();
			return;
		}
		...
		var field = fields[dataset.name].years[doc.year][dataset.data_index],
			...
			value = [ doc.data[dataset.name][field] ];
		...
		counties[doc.FIPS] = counties[doc.FIPS] || {};
		counties[doc.FIPS][doc.year] = value;
	});
}